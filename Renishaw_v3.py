...
class Window(tk.Frame):
    ...
    def init_window(self,):
        ...
        # menu save
        save = tk.Menu(menu)
        save.add_command(label = 'save spectrum', command = self.save_spectrum)
...
#    
    ...
    def save_spectrum(self):
        saving=Saving()
        saving.save_spectrum(self.intensities[:,0],self.lambdas)

class Saving:
    def __init__(self):
        pass
    def save_spectrum(self,spectrum,lambdas):
        name = asksaveasfile(mode='w', defaultextension=".txt")
        if name is None: 
            return
        df=pd.DataFrame({'lambdas':lambdas,'spectrum':spectrum})
...
class Fitting:
    ...
    def fit_perform(self,name,intensities,lambdas,progressbar,status_label,position_x,position_y):
        ...
        map_position,map_FWHM,map_area=self.peak_fitting(intensities,lambdas,fit_data)
        ...
        map_area=map_area[1:].reshape(position_x.shape[0],position_y.shape[0])
        ...
        self.draw_map(map_area,'area '+self.name+'map',position_x,position_y)
    ...
    def peak_fitting(self,intensities,lambdas,fit_data):
        ...
        for i in range (0,intensities.shape[1]):        
                ...
                map_position=np.append(map_position,popt[2])
                map_FWHM=np.append(map_FWHM,popt[3])
                map_area=np.append(map_area,popt[1])
        return map_position,map_FWHM,map_area
